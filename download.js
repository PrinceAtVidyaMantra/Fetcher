...
const generateFilePath = (intervalName, date, baseURL, urls, type) => {
  ...
  if (intervalName === "year") filename = `/y/${year}.json`;
  else if (intervalName === "month") filename = `/m/${year}-${month + 1}.json`;
  else if (intervalName === "day") filename = `/d/${year}-${dayOfYear}.json`;
  else if (intervalName === "hour" && type === "events") filename = `/h/${year}-${dayOfYear}-${hours}.json`;
  else if(intervalName === "hour" && type ==="tags") {return}
  else {
    console.error("Unknown case", intervalName,type, "found");
    return;
  }
...
function generateSyncURLs(lastFetch, currentDate, baseURL, urls) {
  ...
  while (dateItr.getUTCHours() <= 23 && dateItr < currentDate) {
    generateFilePath("hour", dateItr, baseURL, urls);
  ...
  while (dateItr.getUTCDate() <= daysInMonth(dateItr.getUTCMonth(), dateItr.getUTCFullYear()) && dateItr < currentDate) {
    generateFilePath("day", dateItr, baseURL, urls);
  ...
  while (dateItr.getUTCMonth() <= 11 && dateItr < currentDate) {
    generateFilePath("month", dateItr, baseURL, urls);
